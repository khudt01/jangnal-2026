...
작성: 2026-07-03. 상태: 코어 구현 + 콘셉트 A 「잔치 초대장」 스킨 적용 완료(2026-07-04). 사진·장소·인사말 확정분 반영 후 발송.
...
## 6. UI 구조: 장 넘김 프레임 + 콘셉트 A 스킨 + 사운드

인지 부하를 줄이고 초대장을 "한 장씩 넘기는" 경험을 주는 공통 프레임(2026-07-03 위원장 피드백). 콘셉트는 **시안 A 「잔치 초대장」로 확정**(2026-07-04)되어 스킨·실제 음원까지 적용됨.

- **장(step) 구성**: 표지 → 모시는 글 → 행사 안내 → 7년의 발자취 → 참석 여부. 마크업은 `data-step="제목"` 속성으로 선언, JS가 한 장씩만 표시.
- **내비게이션**: JS가 각 장 끝에 「이전 장」(표지 다음 장은 「표지로」) + 「다음 장: <제목>」 버튼을 삽입. 다음 버튼에 목적지 제목을 넣어 위치 감각 제공. 표지에는 「초대장 열기」(1장으로) + 「참석 응답 바로 가기」(마지막 장으로) 2개 진입점.
- **접근성**: 장 전환 시 해당 장의 h1/h2에 포커스 이동(tabindex="-1") + 스크롤 최상단. 숨김은 `hidden` 속성(접근성 트리에서 실제 제거). JS 미동작 시 모든 장이 펼쳐진 한 페이지로 동작(점진적 향상, 표지 링크는 앵커로 폴백).
- **콘셉트 A 스킨(`css/theme.css`)**: 카드형 이중 프레임 + 모서리 매듭 점 4개(`.frame`, 빈 장식 요소), 표지의 Gowun Batang 제목 + 낙관 도장 「일곱돌」(aria-hidden, 정보는 eyebrow 텍스트로 전달), 점·선·마름모 구분 장식(aria-hidden), 단체 대표 사진 자리(사진 확정 후 교체, 캡션은 SR에 노출). 회신 마감 안내는 두지 않는다(참석 응답 기한 없음, 초대장·홍보 글에 신청 기한 표기 금지, 위원장 방침). 본문 장 h2는 명조 + 마름모 장식, 모시는 글 문안은 명조. RSVP 장은 좌측 정렬 유지. 원본 시안: `5. 홍보/초대장 시안/시안A_잔치초대장.html`(내부 폴더).
- **사운드(`js/sound.js`)**: `JangnalSound.play(name)` 공용 API. 효과 3종 모두 실제 음원(Freesound CC0: open=윈드차임 169855, page=페이지 턴 630019, success=성공 차임 541982)을 ffmpeg로 음량 정규화(피크 약 -3dB)·모노 변환해 `assets/sfx/*.mp3`(총 약 57KB)로 탑재. 재생은 HTMLAudioElement(볼륨 0.4~0.5, 스크린 리더 음성 방해 최소화), 실패 시 Web Audio 톤 폴백. 표지의 「소리 켜짐/꺼짐」 토글(상태를 버튼 텍스트로 전달, localStorage 유지). 브라우저 정책상 첫 사용자 조작 이후에만 재생.
- **공유 미리보기·아이콘**: OG 이미지 1200×630(`assets/og.png`, 표지 디자인 기반, 카카오톡 미리보기용) + 파비콘 32px + apple-touch-icon 180px(심볼 로고 기반).
...
- 참석 시에만: 동반 인원(0~3, 활동지원사 등), 편의제공 체크(문자통역/수어통역/기타+자유기술)
- 남기실 말씀(선택, 500자)
...
- 다양한 감각: 사운드 효과 프레임 구축 완료(§6). 위원장 인사말 음성 재생 버튼(선택 재생)은 콘셉트·문안 확정 후 검토.
...
## 10. 남은 작업 (발송 전 확정분 반영)

- 단체 대표 사진 교체(표지 `.photo` 자리), 장소(식당) 확정 반영, 오시는 길(지도 링크)
- 위원장 인사말(모시는 글) 확정 문안 + 음성(선택)
- 프로그램 순서 확정(제11차 중집위) 반영
- (선택) 사운드 3종 청취 검수 후 교체 여부 결정: `assets/sfx/*.mp3`만 갈아끼우면 됨
