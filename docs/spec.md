...
- **콘셉트 A 스킨(`css/theme.css`)**: 카드형 이중 프레임 + 모서리 매듭 점 4개(`.frame`, 빈 장식 요소), 표지의 Gowun Batang 제목 + 낙관 도장 「일곱돌」(aria-hidden, 정보는 eyebrow 텍스트로 전달), 점·선·마름모 구분 장식(aria-hidden), 단체 대표 사진 자리(사진 확정 후 교체, 캡션은 SR에 노출), CTA 아래 회신 마감 안내(7/10 금, 요일 검증 완료). 본문 장 h2는 명조 + 마름모 장식, 모시는 글 문안은 명조. RSVP 장은 좌측 정렬 유지. 원본 시안: `5. 홍보/초대장 시안/시안A_잔치초대장.html`(내부 폴더).
