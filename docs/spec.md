...
## 6. UI 구조: 장 넘김 프레임 + 사운드 (2026-07-03 위원장 피드백 반영)

인지 부하를 줄이고 초대장을 "한 장씩 넘기는" 경험을 주는 공통 프레임. 콘셉트(시안 A/B) 확정 전에 틀만 먼저 구축했고, 스킨·연출·실제 음원은 확정 후 입힌다.
...
- **사운드(`js/sound.js`)**: `JangnalSound.play(name)` 공용 API. 효과 3종: `open`(초대장 열기·바로 가기), `page`(장 넘김), `success`(응답 완료). 현재는 Web Audio 저음량 임시 톤이며 콘셉트 확정 후 실제 음원으로 교체. 표지의 「소리 켜짐/꺼짐」 토글(상태를 버튼 텍스트로 전달, localStorage 유지). 브라우저 정책상 첫 사용자 조작 이후에만 재생.
...
- 사운드 3종(open/page/success)을 콘셉트에 맞는 실제 음원으로 교체 (`js/sound.js`의 SOUNDS만 교체)
- 단체 대표 사진 교체, 장소(식당) 확정 반영, 오시는 길(지도 링크)
- OG 태그 이미지(카카오톡 공유 미리보기) 제작 + 파비콘
